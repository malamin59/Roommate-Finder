...
const PostDetailsPage = () => {
  ...
  const {
    ...
    lifestyle, description, contact, availability, likes = 0
  } = postDetails;
        ...
        <div className="mt-8 flex justify-center">
          <button
            ...
            className={`flex items-center gap-2 px-6 py-3 ${
              isLiked ? 'bg-gray-400' : 'bg-blue-600 hover:bg-blue-700'
...
//   const handleLike = async () => {
//     if (isLiked) return; // prevent multiple likes in one session
...
//         <h2 className="text-4xl font-bold text-center text-blue-700 mb-8">{title}</h2>
...
//             onClick={handleLike}
//             className={`flex items-center gap-2 px-6 py-3 ${
//               isLiked ? 'bg-gray-400 cursor-not-allowed' : 'bg-blue-600 hover:bg-blue-700'
//             } text-white rounded-full shadow-lg transition duration-300`}
//             disabled={isLiked}
//           >
//             <BiLike size={20} />
...
//       if (!res.ok) throw new Error('Failed to update like on server');
...
//             {isLiked ? contact : 'Like the post to see contact'}
//           </p>
//           <p className="flex items-start gap-2">
//             <FaRegStickyNote className="text-indigo-500 mt-1" />
//             <span className="font-semibold">Description:</span>
